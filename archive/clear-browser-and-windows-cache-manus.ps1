...
function Show-Usage {
    param(
        ...
        [System.Management.Automation.PSBoundParametersDictionary]$BoundParameters = $null
    )
    Write-Host ''
    Write-Host '===== Available parameters =====' -ForegroundColor Cyan
    Write-Host '  (Active parameters are highlighted in Green)'
    $color = if ($BoundParameters.ContainsKey('Preview')) { 'Green' } else { 'White' }
    Write-Host '  -Preview                    Show summary only; do not delete files.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('Force')) { 'Green' } else { 'White' }
    Write-Host '  -Force                      Skip the confirmation prompt.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('CloseBrowsers')) { 'Green' } else { 'White' }
    Write-Host '  -CloseBrowsers              Close Firefox/Chrome/Edge/Brave/Opera/Vivaldi before cleanup.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('OlderThanDays')) { 'Green' } else { 'White' }
    Write-Host "  -OlderThanDays <N>          Delete only files older than N days. Default: 0 = all. (Current: $($BoundParameters.OlderThanDays))" -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('IncludeWindowsUpdateCache')) { 'Green' } else { 'White' }
    Write-Host '  -IncludeWindowsUpdateCache  Include Windows Update download/log cache.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('EmptyRecycleBin')) { 'Green' } else { 'White' }
    Write-Host '  -EmptyRecycleBin            Empty the Recycle Bin.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('CleanupComponentStore')) { 'Green' } else { 'White' }
    Write-Host '  -CleanupComponentStore      Run DISM component store cleanup.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('RunDiskCleanup')) { 'Green' } else { 'White' }
    Write-Host '  -RunDiskCleanup             Run cleanmgr /sagerun:100. Configure first with cleanmgr /sageset:100.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('RebuildExplorerCache')) { 'Green' } else { 'White' }
    Write-Host '  -RebuildExplorerCache       Restart Explorer and delete thumbnail/icon cache DB files.' -ForegroundColor $color
    $color = if ($BoundParameters.ContainsKey('ClearDeliveryOptimizationCache')) { 'Green' } else { 'White' }
    Write-Host '  -ClearDeliveryOptimizationCache' -ForegroundColor $color
    Write-Host '                              Clear Windows Delivery Optimization cache.'
    Write-Host ''
...
Show-Usage -BoundParameters $PSBoundParameters
