...
.DESCRIPTION
    Combines the Windows junk sweep and browser cache cleanup flow:
      ...
      2. Finds Firefox profile caches, including the Cache API / service-worker storage that
         lives under the roaming profile (storage\{default,temporary,permanent}\<origin>\cache).
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\bin\clear-browser-and-windows-cache-v6.ps1 -Preview

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\bin\clear-browser-and-windows-cache-v6.ps1 -Force -ClearBrowserCache

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\bin\clear-browser-and-windows-cache-v6.ps1 -DeepWindowsCache -Preview
#>
...
function Show-Usage {
    ...
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -Preview'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -ClearBrowserCache'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -ClearBrowserCache -ForceCloseBrowsers'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -OlderThanDays 30 -IncludeWindowsUpdateCache'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -DeepWindowsCache -Preview'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -RebuildExplorerCache -ClearDeliveryOptimizationCache'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -CleanupComponentStore -ResetBase'
    Write-Host '  .\clear-browser-and-windows-cache-v6.ps1 -Elevate -DeepWindowsCache -Force -Quiet -LogPath C:\logs\clean.log'
}
...
# v6 One summary row covering many folders. Needed for the Firefox Cache API storage, where
# each web origin has its own cache folder: listing 59 origins separately would bury the
# summary. Path keeps the first folder as a representative so the -Unique sort still works.
function Add-CleanupTargetGroup {
    param(
        [Parameter(Mandatory = $true)][string]$Category,
        [Parameter(Mandatory = $true)][string]$Name,
    ...
    $safePaths = [System.Collections.Generic.List[string]]::new()
    foreach ($path in $Paths) {
        $safePath = Resolve-SafeDirectory -Path $path
        if ($safePath) { $safePaths.Add($safePath) }
    }
...
function Add-WindowsTargets {
    ...
    # v6 Windows 11 added a second machine-level temp folder next to Windows\Temp. Same risk
    # profile as Windows Temp (Administrator needed, in-use files are skipped), so it is
    # cleaned by default rather than behind a switch.
    Add-CleanupTarget -Category 'Windows' -Name 'Windows SystemTemp' -Path "$env:WINDIR\SystemTemp"
...
# Firefox splits a profile across two roots: the disk cache sits under LOCALAPPDATA, but the
# Cache API / service-worker storage sits under APPDATA (roaming). v5 only scanned the first
# one, so the storage cache - usually the largest Firefox cache by far - was never cleared.
function Add-FirefoxTargets {
    $localRoot = Join-Path $env:LOCALAPPDATA 'Mozilla\Firefox\Profiles'
    $roamingRoot = Join-Path $env:APPDATA 'Mozilla\Firefox\Profiles'
    # v6 safebrowsing added: a pure download cache that Firefox refetches on demand.
    $cacheNames = @('cache2', 'startupCache', 'thumbnails', 'jumpListCache', 'safebrowsing')

    if (Test-Path -LiteralPath $localRoot -PathType Container) {
        foreach ($profile in Get-ChildItem -LiteralPath $localRoot -Directory -ErrorAction SilentlyContinue) {
    ...
    if (Test-Path -LiteralPath $roamingRoot -PathType Container) {
        foreach ($profile in Get-ChildItem -LiteralPath $roamingRoot -Directory -ErrorAction SilentlyContinue) {
            Add-FirefoxStorageCacheTarget -ProfileName $profile.Name -ProfilePath $profile.FullName
        }
    }
}

# v6 Cache API / service-worker cache: <roaming profile>\storage\<bucket>\<origin>\cache
#
# Only the 'cache' subfolder of each origin is collected. The sibling folders idb and ls hold
# IndexedDB and localStorage - real site data - so deleting a whole origin folder would log the
# user out of sites and lose their data. That is exactly the cost we refused when we decided
# not to clear cookies, so the origin folder itself is never a target.
function Add-FirefoxStorageCacheTarget {
    param(
    ...
    foreach ($bucket in @('default', 'temporary', 'permanent')) {
        $bucketPath = Join-Path $ProfilePath (Join-Path 'storage' $bucket)
        if (-not (Test-Path -LiteralPath $bucketPath -PathType Container)) { continue }

        foreach ($origin in Get-ChildItem -LiteralPath $bucketPath -Directory -ErrorAction SilentlyContinue) {
            $originCache = Join-Path $origin.FullName 'cache'
            if (Test-Path -LiteralPath $originCache -PathType Container) {
                $cachePaths.Add($originCache)
            }
        }
    }

    if ($cachePaths.Count -eq 0) { return }

    $label = '{0} / storage cache ({1} origins)' -f $ProfileName, $cachePaths.Count
    Add-CleanupTargetGroup -Category 'Firefox' -Name $label -Paths $cachePaths.ToArray()
}
...
function Remove-TargetContents {
    ...
    # v6 A target can now span several folders (Add-CleanupTargetGroup). Single-folder targets
    # carry a one-element Paths list, so both kinds go through the same loop. The fallback to
    # .Path keeps this working if a target is ever built without Paths.
    ...
    foreach ($path in $paths) {
        ...
        Get-ChildItem -LiteralPath $path -Include $Target.Include -Recurse -Force -File -ErrorAction SilentlyContinue |
            Where-Object { -not $cutoff -or $_.LastWriteTime -lt $cutoff } |
            ForEach-Object {
                $len = $_.Length
                try {
