...
.DESCRIPTION
    ...
    Close browsers first for the best result, or use -CloseBrowsers / -ForceCloseBrowsers.
...
.PARAMETER CloseBrowsers
    Close supported browsers before cleanup: request a graceful shutdown first, then
    force-kill only the processes that do not exit in time.

.PARAMETER ForceCloseBrowsers
    Immediately force-kill supported browsers (no graceful shutdown). Overrides -CloseBrowsers.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\ps1\clear-browser-and-windows-cache-v2.ps1 -Preview

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\ps1\clear-browser-and-windows-cache-v2.ps1 -Force -CloseBrowsers

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File C:\opt\ps1\clear-browser-and-windows-cache-v2.ps1 -DeepWindowsCache -Preview
#>
...
$ParameterInfo = @(
    ...
    @{ Name = 'CloseBrowsers'; Type = 'Switch'; Description = 'Graceful browser shutdown, then force-kill leftovers.' }
    @{ Name = 'ForceCloseBrowsers'; Type = 'Switch'; Description = 'Immediately force-kill browsers.' }
...
function Show-Usage {
    ...
    Write-Host '  .\clear-browser-and-windows-cache-v2.ps1 -Preview'
    Write-Host '  .\clear-browser-and-windows-cache-v2.ps1 -Force -CloseBrowsers'
    Write-Host '  .\clear-browser-and-windows-cache-v2.ps1 -OlderThanDays 30 -IncludeWindowsUpdateCache'
    Write-Host '  .\clear-browser-and-windows-cache-v2.ps1 -DeepWindowsCache -Preview'
    Write-Host '  .\clear-browser-and-windows-cache-v2.ps1 -RebuildExplorerCache -ClearDeliveryOptimizationCache'
}

# #2 Warn about conflicting or meaningless option combinations.
function Show-CombinationWarnings {
    param([bool]$BrowsersRunning)
    ...
    if ($CloseBrowsers -and $ForceCloseBrowsers) {
        $notes.Add(@('-CloseBrowsers is ignored because -ForceCloseBrowsers takes priority.', 'Drop -CloseBrowsers, or drop -ForceCloseBrowsers for a graceful shutdown first.'))
    }
    if ($Preview -and ($CloseBrowsers -or $ForceCloseBrowsers)) {
        $notes.Add(@('Browsers are not closed in Preview mode.', 'Remove -Preview to actually close browsers and clean.'))
    }
    if ($BrowsersRunning -and -not $Preview -and -not $CloseBrowsers -and -not $ForceCloseBrowsers) {
        $notes.Add(@('Browsers are still running.', 'Use -CloseBrowsers for a more complete cleanup.'))
    }
...
function Test-BrowsersRunning {
    $procs = Get-Process -Name $script:BrowserProcessMap.Values -ErrorAction SilentlyContinue
    return [bool]$procs
}
...
$browsersRunning = Test-BrowsersRunning

# #2 Point out conflicting / meaningless option combinations.
Show-CombinationWarnings -BrowsersRunning $browsersRunning
