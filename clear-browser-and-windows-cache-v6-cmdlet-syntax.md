# clear-browser-and-windows-cache-v6.ps1 — 사용된 내장 함수 / 문법 설명

이 문서는 `clear-browser-and-windows-cache-v6.ps1` 에서 실제로 쓰인 PowerShell
...
**외부 실행 파일 호출**
- `& dism.exe @dismArgs` — `&`(호출 연산자)로 exe 실행, 인자는 배열을 **splatting** 으로 전달
  (아래 10장 참고).
...
| `[System.Collections.Generic.List[string]]` | 문자열 리스트. `.ToArray()` 로 배열 변환 | `$cachePaths.ToArray()` |
| `[Math]::Min / Max` | 수치 min/max | `[Math]::Max(0L, $freeAfter - $freeBefore)` |
| `[pscustomobject]@{ ... }` | 즉석 구조체(속성 묶음) 생성 | `[pscustomobject]@{ Count=...; Bytes=... }` |
| `.PSObject.Properties['이름']` | 객체에 그 속성이 **있는지** 확인(v6) | `$Target.PSObject.Properties['Paths']` |
...
**`.PSObject` — 객체 자기 자신에 대한 정보 (v6 에서 추가 사용)**

`[pscustomobject]` 는 속성 구성이 제각각일 수 있습니다. 없는 속성을 그냥 읽으면 오류가 아니라
`$null` 이 나오므로, "속성이 존재하는가" 와 "값이 비었는가" 를 구분하려면 `.PSObject` 를 씁니다.

```powershell
# v6: 타겟이 여러 폴더를 담은 그룹인지, 옛 방식의 단일 폴더인지 판별
$paths = if ($Target.PSObject.Properties['Paths'] -and $Target.Paths) {
    @($Target.Paths)
} else {
    @($Target.Path)     # Paths 가 없는 타겟이 생겨도 안전하게 폴백
}
```

- `.PSObject.Properties` : 속성 목록(이름으로 인덱싱 가능). 없으면 `$null`.
- `@(...)` 로 감싸는 이유: 원소가 1개면 PowerShell 이 배열을 벗겨 스칼라로 만들기 때문에,
  `foreach` 로 돌리려면 **배열임을 강제**해야 합니다.
...
`$변수` 대신 **`@변수`** 로 쓰면 컬렉션이 "여러 개의 인자"로 펼쳐져 전달됩니다.
조건에 따라 인자를 바꿔야 할 때 문자열을 조립하지 않아도 되어 안전합니다.

```powershell
# v5: /ResetBase 를 조건부로 붙여 dism.exe 실행
...
- **배열 스플래팅**(위 예시): 각 요소가 **위치 인자**로 전달. 외부 exe 호출에 적합.
- **해시테이블 스플래팅**: 키가 **파라미터 이름**, 값이 그 값으로 전달. cmdlet 호출에 적합.
  ```powershell
  $p = @{ LiteralPath = $path; Force = $true; ErrorAction = 'Stop' }
  Remove-Item @p          # = Remove-Item -LiteralPath $path -Force -ErrorAction Stop
  ```
- `@dismArgs` 와 `$dismArgs` 의 차이: 후자를 그대로 넘기면 **배열 하나**가 단일 인자로
  전달되어 의도와 달라질 수 있습니다. 반드시 `@` 를 사용합니다.
- 주의: 스플래팅 기호는 `@` 이지만, 배열/해시 **리터럴**의 `@(...)`, `@{...}` 와는 다른
  문법입니다(같은 기호를 다른 의미로 씁니다).
...
- **오타 추천**: 미바인딩 인자를 `ValueFromRemainingArguments` 로 받아 Levenshtein 거리로 유사
  파라미터 제안.
- **조건부 외부 명령 인자**: 인자를 배열에 모아 조건에 따라 `+=` 로 추가하고 `@배열` 스플래팅으로
  전달(`dism.exe` 의 `/ResetBase`). 문자열 조립보다 안전하고 읽기 쉬움.
- **속성 하나로 단일/복수 통일 (v6)**: 타겟 객체에 항상 `Paths` 배열을 넣어 단일 폴더도
  1개짜리 목록으로 만들고, 삭제 루프는 `foreach ($path in $paths)` 하나로 처리.
  분기를 삭제 코드에 흩뿌리지 않고 **데이터 모양을 통일**하는 방식.
- **표시 단위와 삭제 단위 분리 (v6)**: Firefox 스토리지 캐시는 폴더 59개를 지우지만 요약에는
  한 줄로 보인다. `Path`(대표) + `Paths`(전체)를 함께 들고 있어, 정렬/중복제거는 대표 경로로,
  실제 삭제는 전체 목록으로 수행.
- **형제 폴더 보호 (v6)**: 부모 폴더를 통째로 지우지 않고 **이름이 정확히 맞는 하위 폴더만**
  대상에 넣는다(`<origin>\cache` 만, 옆의 `idb`/`ls` 는 제외). 대상 선정 단계에서 막으면
  삭제 코드에 예외 처리를 넣지 않아도 된다.

---

## 참고
- cmdlet 상세는 PowerShell에서 `Get-Help <cmdlet> -Full` 또는 `Get-Help <cmdlet> -Online`.
- 이 스크립트 자체 도움말: `Get-Help .\clear-browser-and-windows-cache-v6.ps1 -Full`.
